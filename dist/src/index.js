 ...
 */
const kindDrawOrder = ["ground", "spawner", "goodGuy", "spaghettiMonster", "ignoranceSpreader", "ignorant"];
/**
...
function displayWorldToGrid(world, actors, grid, mainSpaghettiMonster) {
    ...
    const actorsInDrawOrder = kindDrawOrder.reduce((acc, kind) => acc.concat(actors.filter((a) => hasOneOfKinds(a, [kind]))), []);
...
async function main() {
    ...
    const world = initWorld(squareWorldSize, squareWorldSize);
    ...
    let actors = initActors(world, intermediateWaypointLinesNumber, spawnersAxis, spawnProba, playProba, minSpawners, maxSpawners);
