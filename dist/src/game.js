...
function initGroundWaypoints(world, minGroundsPerLine, maxGroundsPerLine, groundsAxis, groundLineNumbers, numberOfGroundLines) {
    return Array.from({ length: numberOfGroundLines }, (_, index) => (randomPositionsAlongAxis(world, minGroundsPerLine, maxGroundsPerLine, groundsAxis, groundLineNumbers[index])
...
function initWayPointActors(world, intermediateWaypointsNumber, spawnersAxis, averageSpawnsPerPhase, minSpawners = 1, maxSpawners = 3) {
    ...
    const intermediateWaypointsLineNumber = almostEvenlySpacedIntegers(intermediateWaypointsNumber, spaghettiMonsterLineNumber ? 0 : maxLineNumber, spaghettiMonsterLineNumber);
...
function resolveProposals(world, actors, proposals) {
    return proposals.reduce((acc, currentProposal, actorIndex) => {
        if (currentProposal === undefined)
            throw new Error("undefined actor");
...
function nextTurn(phases, world, actors, spawnersAxis) {
    return phases.reduce((actorsAcc, aPhase) => {
        const proposals = aPhase.executePhase(actorsAcc, actorsAcc.map((actingActor) => actingActor.actions[aPhase.funcName]({ actorsAcc, actingActor, world, spawnersAxis }) /* ReturnType<ActorActions[keyof ActorActions]> */));
