...
import { setConviction, setFaithPoints, setMaxFaith, setSpawnProba, setWaypointNumber, setWaypointTargetAndNumber, setFaithPointsAndMax, setSpreadIgnorancePower, setRange, setPlayProba } from "./props";
/**
 * Actor constructor
...
function createActor(position, actionGenerators, actions, kind, externalProps) {
    const actorActions = { ...defaultActions, ...actions };
    ...
    const actorActionsGenerators = { ...defaultActionsGenerators, ...actionGenerators };
...
function createIgnorant(position, waypointTarget, faithPoints = 100) {
    ...
    return setWaypointTargetAndNumber(setFaithPointsAndMax(setConviction(createActor(position, { move: executeFunctionEveryNCall(moveTowardWaypointTarget, defaultActions["move"], 2) }, { move: moveTowardWaypointTarget, temperatureRise, enemyFlee }, "ignorant"), 10), faithPoints, 100), waypointTarget, 1);
}
/**
...
function createIgnoranceSpreader(position, waypointTarget, faithPoints = 70, spreadIgnorancePower = 5, range = 3) {
    throwErrorIfUndefined(waypointTarget);
    return setWaypointTargetAndNumber(setFaithPointsAndMax(setSpreadIgnorancePower(setRange(createActor(position, {}, { move: moveTowardWaypointTarget, spreadIgnorance, enemyFlee }, "ignoranceSpreader"), range), spreadIgnorancePower), faithPoints, 70), waypointTarget, 1);
}
/**
...
function createSpawner(position, spawnProba = 0.3) {
    return setSpawnProba(setWaypointNumber(createActor(position, {}, { spawn }, "spawner"), 0), spawnProba);
}
/**
...
function createGoodGuy(position, range = 2, conviction = 9) {
    return setRange(setConviction(createActor(position, {}, { convertEnemies }, "goodGuy"), conviction), range);
}
/**
...
function createSpaghettiMonster(position, waypointNumber, faithPoints = 100) {
    return setWaypointNumber(setFaithPoints(setMaxFaith(createActor(position, {}, {}, "spaghettiMonster"), 100), faithPoints), waypointNumber);
}
/**
...
function createPlayer(playProba = 0.25) {
    return setPlayProba(createActor(createVector(0, 0), {}, { play: play }, "player"), playProba);
}
...
//# sourceMappingURL=actor_creators.js.map